...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        $stmt->execute([$user['id'], $amount, "ATM Deposit - $description"]);

        // Notification
        $stmt = $pdo->prepare("INSERT INTO notifications (user_id, message, read_status) VALUES (?, ?, 0)");
        $stmt->execute([$user['id'], "ATM deposit of AUD " . number_format($amount,2) . " completed."]);
...
<html>
<head>
    <title>ATM Deposit | Southern Cashflow Finance</title>
    <style>
...
<div class="container">
    ...
    <form method="POST">
        ...
        <textarea name="description" rows="3" placeholder="Salary, savings, family support..." required></textarea>
